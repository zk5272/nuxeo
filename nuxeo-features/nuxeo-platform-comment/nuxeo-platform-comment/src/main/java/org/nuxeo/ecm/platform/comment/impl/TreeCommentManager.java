...
public class TreeCommentManager extends AbstractCommentManager {
    ...
    public Comment createComment(CoreSession session, Comment comment) {
        ...
        return CoreInstance.doPrivileged(session, s -> {
            ...
            DocumentRef locationDocRef = getDocumentRefContainerOfCommentCreation(s, commentedDoc);
    ...
    public DocumentModel createComment(DocumentModel commentedDoc, DocumentModel commentDoc) {
        ...
        return CoreInstance.doPrivileged(commentDoc.getCoreSession(), session -> {
            ...
            DocumentRef locationDocRef = getDocumentRefContainerOfCommentCreation(session, commentedDoc);
    ...
    public String getLocationOfCommentCreation(CoreSession session, DocumentModel commentedDoc) {
        return getDocumentModelContainerOfCommentCreation(session, commentedDoc).getPathAsString();
    }

    @Override
    public DocumentRef getDocumentRefContainerOfCommentCreation(CoreSession session, DocumentModel commentedDocModel) {
        return getDocumentModelContainerOfCommentCreation(session, commentedDocModel).getRef();
    }

    /**
     * Returns the {@link DocumentModel} container of the comments for the given commented document model.
     *
     ...
     * @since 10.10-HF23
     */
    protected DocumentModel getDocumentModelContainerOfCommentCreation(CoreSession session,
